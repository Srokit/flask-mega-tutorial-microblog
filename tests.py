...
class TestUser(unittest.TestCase):
    ...
    def test_avatar(self):
        ...
        assert avatar[0:len(expected_url)] == expected_url
    ...
    def test_follow(self):
        ...
        db.session.add(u2)
        db.session.commit()
        # Do not need to add u1 to the database because unfollowing from
        #  a user that you do not yet follow does nothing
        assert u1.unfollow(u2) is None
        u = u1.follow(u2)
        db.session.add(u)
        db.session.commit()
        # Following a user that you already follow does nothing
    ...
    def test_followed_posts(self):
        # I just copied this from the tutorial #YOLO
        u1 = User(nickname='john', email='john@example.com')
        u2 = User(nickname='susan', email='susan@example.com')
